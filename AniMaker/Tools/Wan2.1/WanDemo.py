...
class WanT2V14B:
    ...
    def __init__(self):
        self.ckpt_dir = "Tools/Wan2.1/Wan2.1-T2V-14B" 
        self.model = None
        
    def _init_model(self):
        """Initialize the model with the given checkpoint directory"""
        ...
        # Apply TeaCache
        self.model.__class__.generate = t2v_generate
        self.model.model.__class__.enable_teacache = True
        self.model.model.__class__.forward = teacache_forward
        self.model.model.__class__.cnt = 0
        self.model.model.__class__.num_steps = 50*2  # Default sample_steps for t2v is 50
        self.model.model.__class__.teacache_thresh = 0.2
        self.model.model.__class__.accumulated_rel_l1_distance_even = 0
        self.model.model.__class__.accumulated_rel_l1_distance_odd = 0
        self.model.model.__class__.previous_e0_even = None
        self.model.model.__class__.previous_e0_odd = None
        self.model.model.__class__.previous_residual_even = None
        self.model.model.__class__.previous_residual_odd = None
        self.model.model.__class__.use_ref_steps = False
        
        # Coefficients for 14B model
        self.model.model.__class__.coefficients = [-5784.54975374, 5449.50911966, -1811.16591783, 256.27178429, -13.02252404]
        self.model.model.__class__.ret_steps = 2
        self.model.model.__class__.cutoff_steps = 50*2 - 2
    
    def generate(self, prompt, size="1280*720", frame_num=81, shift=5.0, sample_solver='unipc', sampling_steps=50, guide_scale=5.0, save_file=None):
        ...
        if save_file is not None:
            ...
            cache_video(
                tensor=video[None],
                save_file=save_file,
                fps=self.model.config.sample_fps,
                nrow=1,
                normalize=True,
                value_range=(-1, 1)
            )
...
class WanI2V14B:
    ...
    def __init__(self):
        self.ckpt_dir = "Tools/Wan2.1/Wan2.1-I2V-14B-480P" 
        self.model = None
    ...
    def _init_model(self):
        ...
        # Apply TeaCache
        self.model.__class__.generate = i2v_generate
        self.model.model.__class__.enable_teacache = True
        self.model.model.__class__.forward = teacache_forward
        self.model.model.__class__.cnt = 0
        self.model.model.__class__.num_steps = 40*2  # Default sample_steps for i2v is 40
        self.model.model.__class__.teacache_thresh = 0.2
        self.model.model.__class__.accumulated_rel_l1_distance_even = 0
        self.model.model.__class__.accumulated_rel_l1_distance_odd = 0
        self.model.model.__class__.previous_e0_even = None
        ...
        # Coefficients for I2V 480P model
        self.model.model.__class__.coefficients = [-3.02331670e+02, 2.23948934e+02, -5.25463970e+01, 5.87348440e+00, -2.01973289e-01]
        self.model.model.__class__.ret_steps = 2
        self.model.model.__class__.cutoff_steps = 40*2 - 2
    
    def generate(self, prompt, image_path, max_area=832*480, frame_num=81, shift=5.0, sample_solver='unipc', sampling_steps=40, guide_scale=5.0, seed=-1, save_file=None):
        ...
        # Generate video
        logging.info(f"Generating video with prompt: {prompt}")
        logging.info(f"Using input image: {image_path}")
...
# Modify the __main__ block to include a command line flag for using the I2V model
if __name__ == "__main__":
    # t2v_model = WanT2V14B()
    # video = t2v_model.generate(
    #     prompt="Two anthropomorphic piggys in comfy boxing gear and bright gloves fight intensely on a spotlighted stage.",
    #     save_file="Pipeline/imgs/environments/output_t2v.mp4"
    # )
    ...
    i2V_model = WanI2V14B()
    image_path = "Tools/Wan2.1/demo/scene_1_clip_4_2s_h1.jpg"
    prompt = "The girl turns around and walks towards the door, 3D animation." 
    output_file1 = "Tools/Wan2.1/demo/outputs/69-1-4_h2-7.mp4"
    output_file2 = "Tools/Wan2.1/demo/outputs/69-1-4_h2-8.mp4"
    output_file3 = "Tools/Wan2.1/demo/outputs/69-1-4_h2-9.mp4"
    video = i2V_model.generate(
        prompt=prompt,
        image_path=image_path,
        save_file=output_file1,
        shift=5.0,
        guide_scale=7.5
    )
